...
class ResourceMonitor():
    ...
    def _create_ssl(self):
        context = self.__create_ssl_context(
        ...
        return context
    def __create_ssl_context(self, *, cafile=None, capath=None, cadata=None,
        ...
        """
        if cafile or capath:
            self.logMessage('Loading SSL CA from %s', cafile or capath)
        elif cadata is not None:
            self.logMessage('Loading SSL CA from data provided in `cadata`')
            self.logMessage('`cadata`: %r', cadata)
        ...
        if certfile is not None:
            self.logMessage('Loading SSL Cert from %s', certfile)
            if keyfile:
                if password is not None:
                    self.logMessage('Loading SSL Key from %s with password', keyfile)
                else:  # pragma: no cover
                    self.logMessage('Loading SSL Key from %s without password', keyfile)
            # NOTE: From docs:
